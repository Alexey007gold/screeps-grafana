...
def file_fetch(fh, fromTime, untilTime, now=None, archiveToSelect=None):
  ...
  # When the query range exceeds archive retention, circular buffer modular addressing
  # maps fromTime/untilTime to wrong slot positions (e.g. "Last 5 years" on a 2y archive).
  # Read baseInterval (the timestamp stored in slot 0) and clamp the query so that
  # fromInterval == baseInterval, which makes slot offset math land correctly on actual data.
  if not archiveToSelect and diff > archive['retention']:
    archiveStep = archive['secondsPerPoint']
    fh.seek(archive['offset'])
    try:
      (baseInterval, _) = struct.unpack(pointFormat, fh.read(pointSize))
    except struct.error:
      baseInterval = 0
    if baseInterval > 0:
      if fromTime < baseInterval - archiveStep:
        fromTime = baseInterval - archiveStep
      archiveEnd = baseInterval + archive['retention'] - archiveStep
      if untilTime > archiveEnd:
        untilTime = archiveEnd
      if fromTime > untilTime:
        return None
